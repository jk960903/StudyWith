	...
	public static void main(String[] args) {
		...
		for(int num = 65 ; num <= 90; num++){
			...
			for(int i = 0 ; i < length; i++) {
				if((int)speech.charAt(i) == num || (int)speech.charAt(i) == num+32 ) {
					count++;
				}
				if(((int)speech.charAt(i) >= 65 && (int)speech.charAt(i) <=90) ||( (int)speech.charAt(i) >= 97 && (int) speech.charAt(i) <= 122)) {
					countAlpha++;
				}
			}
			double percent = ((double)count / (double)countAlpha) * 100;
			String per = String.format("%.2f" , percent);
			if(count <10) {
				System.out.println((char)num +":   "+count+"개 ,   "+per+"%");
			}else if(count<100) {
				System.out.println((char)num +":  "+count+"개 ,   "+per+"%");
			}else {
				System.out.println((char)num +": "+count+"개 ,   "+per+"%");
